...
public interface Traversal<T> {
	...
	interface Provider<T, F> {
		...
		default Traversal<T> bubbleUp(Traversal<T> base) {
			return scope -> hierarchy(scope).map(t -> debug("bubbling up " + t, base.hop(t)))
		 ...
		 *
		 * The traversal attempts to find a base object following the following strategy:
		 * <ol>
		 *     <li>a child in the current context that has the given feature (attribute or relationship) with
		 *     the first value given</li>
		 *     <li>if no child is found with the given value for the given feature, attempt the first step
		 *     having each of the parent scopes as context</li>
		 *     <li>for each additional value to match, the child with the given feature.</li>
		 * </ol>
		 *
		...
		default Traversal<T> search(F feature, int offset, Object... path) {
			...
			// 4. any root
			var findParentOrRelative = any(attributeValued(feature, path[offset]), childWithAttributeValued(feature, path[offset]));
